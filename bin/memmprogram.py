...
for cmd in sort_list:
    print( "%8sB + %8sB = %8sB\t%s" % (human(cmd[1]-shareds[cmd[0]]), human(shareds[cmd[0]]), human(cmd[1]), cmd_with_count(cmd[0], count[cmd[0]])))
    tot_priv = tot_priv + (cmd[1]-shareds[cmd[0]])
    tot_shared = tot_shared + (shareds[cmd[0]])

# Total
print( "\n%8sB + %8sB = %8sB" % (human(tot_priv), human(tot_shared), human(tot_priv+tot_shared)))
print(" Private  +  Shared  =  RAM used\tProgram")
